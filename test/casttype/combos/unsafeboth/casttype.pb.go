...
func (m *Castaway) MarshalTo(data []byte) (int, error) {
	...
	if m.MyFloat32Ptr != nil {
		data[i] = 0x2d
		i++
		*(*float32)(unsafe.Pointer(&data[i])) = *m.MyFloat32Ptr
		i += 4
	}
	data[i] = 0x35
	i++
	*(*float32)(unsafe.Pointer(&data[i])) = m.MyFloat32
	i += 4
	if m.MyFloat64Ptr != nil {
		data[i] = 0x39
		i++
		*(*float64)(unsafe.Pointer(&data[i])) = *m.MyFloat64Ptr
		i += 8
	}
	data[i] = 0x41
	i++
	*(*float64)(unsafe.Pointer(&data[i])) = m.MyFloat64
	i += 8
	if m.MyBytes != nil {
		data[i] = 0x4a
		i++
